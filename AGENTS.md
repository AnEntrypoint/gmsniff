...
**Naming an omission does not make it reachable.** `--stats` correctly printed "+51 more groups not shown (list caps at 20)" while hardcoding every cap and ignoring `--top`, so the count was reachable and the rows were not — an honest notice pointing at 51 projects no flag could display. When a list caps, check that the escape hatch it implies actually works.

**An aggregate that cannot name its producer answers the wrong question.** `--stats` grouped by `sub`, `event`, `sess` and `day` but never by `cwd`, so `C:/dev/design` emitting **15,065** `config_resolved` events at 3.3/s — against `spoint`'s 19 and `gm`'s 23 — vanished inside one fleet-wide `config_resolved 15410` line. For a tool whose product is *which agent is doing what*, per-project attribution is the axis, not an extra. Check feasibility before designing around a gap: the raw log carries `cwd` on only 73% of records, but the replay layer attaches it per source log, so stored events were already complete. And assert the reconciliation — per-project buckets must sum to the fleet total, with anything unattributed in its own named bucket, never folded into a real project.

**Two surfaces measuring one thing will drift, so give them one implementation.** `/api/spool-queue` walked `in/` itself rather than calling `spoolQueueDepth`, and the copies diverged: the route reported **35 pending** for the tree the CLI reported as **0 pending, 34 unconsumable**, and it rendered a stray `.gm/` dot-directory as a verb queue because it filtered dot-*files* but not dot-*directories*. A reader facing two numbers for one filesystem cannot tell which to trust, which is worse than either being wrong alone. Fix the duplication, not the symptom — patching the route in place would have re-drifted the moment either copy changed. When a shared counter must serve a second caller, widen its return value (here, a per-verb breakdown) rather than letting the caller write its own.

**A count over a mixed population reports neither half.** `queue_depth` counted every file under `in/`, but the spool ABI is `in/<verb>/<N>.txt` — anything else is never consumed and the number can never reach zero. Measured across the fleet, *every* agent read **0 genuinely pending**: `spoint` showed 34 unconsumable with the oldest at 54 days, `gm` 19, `rs-plugkit` 20, `thebird` 2 at 1,126 hours. The column had never once shown a real backlog, yet rendered identically to one. Before displaying a count, ask whether the things counted are the same kind of thing; if not, report each with the measurement that distinguishes them (here, the oldest residue's age). And when a widely-read field must gain structure, add the breakdown *beside* it rather than changing its type — five surfaces consumed `queue_depth` as a number.
...
Phases are SPECIFY, PROVE, EMIT, STATE, CONC, SEC, RES, DECIDE, COMPLETE. The graph has forward-gated edges (PROVE->EMIT, EMIT->STATE, STATE->CONC, SEC->RES, RES->DECIDE, DECIDE->COMPLETE) plus **gate-free re-plan/repair/boundary-enforcement edges**: PROVE/EMIT/STATE/SEC/RES/DECIDE->SPECIFY (reshaping), DECIDE->PROVE (empirical fitness), STATE/CONC/SEC/RES->EMIT (repair), CONC/SEC->STATE (boundary enforcement) — so a phase can legally be revisited out of forward order. Any phase-walk display that computes gaps by linear index math will emit false "skipped phase" reports; walk the real edges (any edge landing on an earlier phase in the sequence than the one it left is a legal re-plan/repair edge, never a skip). The seventeen gates are `residual-scan-fired`, `prd-all-closed`, `mutables-all-resolved`, `worktree-clean`, `ci-validated-fresh`, `browser-witness-coverage`, `claim-audit-clean`, `submodules-clean`, `app-loads-witnessed`, `no-synthetic-test-files`, `no-admit-deferral-markers`, `no-secrets-in-diff`, `no-unchecked-panics-in-diff`, `no-hedge-language-in-diff`, `no-graphical-symbols-in-diff`, `idempotent-dispatch-replay-safe`. A project's `.gm/instructions/fsm/graph.json` can override the graph per-project, so read the project's own graph when one exists rather than assuming the default.
