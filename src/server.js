...
import {
  ...
  readDaemonStatus, readDaemonStatusFile, readInstalledVersions, spoolQueueDepth, daemonPoolAndPollFields,
} from './registry.js';
...
function readDaemonStatusGlobal() {
  ...
  const file = readDaemonStatusFile(fp);
  const j = file.json;
  if (!j) {
    const alert = file.unreadable ? 'daemon heartbeat file present but not parseable (a torn write, or a truncated file)' : null;
    return { present: file.present, unreadable: file.unreadable, pid: null, ts: null, active_projects: null, age_ms: null, stale: true, alert, ...daemonPoolAndPollFields(null) };
  }
  ...
  const alert = stale
    ...
    : null;
  return {
    present: true, unreadable: false, pid: j.pid ?? null, pid_alive: pidAlive, ts,
...
const API_ROUTES = [
  ...
  { path: '/api/daemon', method: 'GET', params: [], response: '{present, unreadable, pid, pid_alive, ts, active_projects, age_ms, stale, stale_threshold_ms, alert, pool_utilization, mixed_version_pools, pending_store_swaps, last_plugin_update_poll_error, last_plugin_update_poll_ts, last_runner_update_poll_error, last_runner_update_poll_ts, memory, memory_reported, plugin_store_bytes, shared_dispatches_since_release, last_shared_store_release}. Machine-global shared-daemon heartbeat (~/.agentplug/daemon-status.json). unreadable=true means the file exists but did not parse (torn write). memory is the runner process breakdown {rss_bytes, anon_bytes, file_bytes, shmem_bytes, swap_bytes, private_bytes} or null when the runner does not report it (memory_reported=false); plugin_store_bytes is {plugin: {instances, total_bytes, max_bytes, ceiling_bytes}} for every live wasm Store; last_shared_store_release is the last release record {ts, age_ms, trigger, reason, released, private_bytes_before, private_bytes_after} or null. pool_utilization is {plugin_name: {pool_size, occupied}} per shared Store pool; occupied is a point-in-time slot count, not a verdict on contention.' },
